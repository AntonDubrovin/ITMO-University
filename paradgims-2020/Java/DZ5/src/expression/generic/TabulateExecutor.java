...
public class TabulateExecutor<T extends Number> {
    private Function<String, Type> parser;

    public TabulateExecutor(Function<String, Type> parser) {
        this.parser = parser;
    }

    public Object[][][] execute(String expr, int x1, int x2, int y1, int y2, int z1, int z2) {
        TripleExpression<T> expression = new ExpressionParser<T>(parser).parse(expr);
        int deltaX = x2 - x1;
        int deltaY = y2 - y1;
        int deltaZ = z2 - z1;
